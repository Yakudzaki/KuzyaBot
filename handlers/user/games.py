...
botik = [
    ...
    "Опять на передовой?", "Не устаю служить.", "Какие приказы?", "Новые приказы?", "Что важно делать?",
    "Снова на связи.", "Да, мой господин.", "Какие распоряжения?", "Позвали?", "Чем бы вы хотели заняться?", 
    "Что по заданию?", "Пришёл для выполнения приказов.", "Чем могу помочь в этот раз?", "Доступен для указаний.",
    "Какие инструкции?", "Чем заняться сейчас?", "Опять работаем?", "Что требуется?", "Жду указаний.", "Что следует предпринять?", 
    "Какие указания для меня?", "Готов к действиям!", "Снова на службе.", "Чем полезен в этот раз?", "Какие новости?", "Есть задача?", 
    "Чем вам помочь?", "Приветствую вас.", "Какие дела?", "Готов выполнить поручение.", "Слушаю ваши указания.", "Новые инструкции?",
    "Есть ли что-то важное?", "Что нужно сделать?", "Ваше желание - моя команда.", "Что по новому?", "Какие требования?"
]
...
botik_angry = ["Неужели это всё, что ты можешь сказать?", "Попробуй не раздражать меня!",
    ...
    "Не стоит меня так раздражать!", "Может, хватит уже?", "Ты не устал?", "Заткнись, пожалуйста.", "Ты меня бесишь!", 
    "Я не понимаю, как можно быть таким назойливым.", "Похоже, ты нацелен на то, чтобы меня разозлить.", 
    "Ты уверен, что хочешь продолжать эту беседу?", "Давай пообщаемся, когда ты успокоишься."]
...
#Отзыв ботика
    ...
    if message.text.lower().startswith('кузя кто ') or message.text.lower().startswith('кузя, кто '):
        ...
        answers = [
            f"🔮 Ясно вижу, что {user} {who}",
            f"☝ Я уверен, что  {user} {who}",
            f"🎱 Шар говорит, что {user} {who}",
            f"💫 Звезды говорят, что {user} {who}",
            f"🧐 Я думаю, что {user} {who}",
            f"💭 Предчувствую, что {user} {who}",
            f"🔍 По моим наблюдениям, {user} {who}",
            f"🔭 Мой анализ говорит, что {user} {who}",
            f"🔬 Моя интуиция подсказывает, что {user} {who}",
            f"{user}, {who}",
            f"Это {user}, {who}",
            f"{user}",
            f"👨‍🔬 Согласно моим расчётам, {user} {who}",
            f"🤔 Судя по всему, {user} {who}",
            f"🤫 Мне кажется, что {user} {who}",
            f"🧐 На мой взгляд, {user} {who}",
            f"😌 По моему мнению, {user} {who}",
            f"🤨 В моём представлении, {user} {who}",
            f"🕵️ Вероятно, {user} {who}",
            f"🤓 По всей видимости, {user} {who}",
            f"💯 Похоже, {user} {who}",
            f"🗣️ Как мне кажется, {user} {who}",
            f"🔎 Похоже на то, что {user} {who}",
            f"🤷‍♂️ Наверное, {user} {who}",
            f"🙊 Я бы сказал, что {user} {who}",
            f"👀 По моим предположениям, {user} {who}",
            f"✍️ Мой вывод таков, что {user} {who}",
            f"📢 Вероятно, {user} {who}",
            f"🤐 Похоже, {user} {who}"
        ]
...
#ПОНГ_КОНГ_ДУНКАН
    if message.text.lower() == "пинг" and cor_tx <= 2:
        await bot.send_chat_action(message.chat.id, types.ChatActions.TYPING)
        a = time.time()
        bot_message = await message.reply(f'⚙ Проверка пинга....')
        if bot_message:
              b = time.time()
    ...
    if message.text.lower() == "пиу" and cor_tx <= 2:
        await bot.send_chat_action(message.chat.id, types.ChatActions.TYPING)
...
async def ladna_func(message: types.Message, word):
    ...
    if user[0] == yakudza_id:
        rpword = ["делает жарко", "нагревает", "утепляет", "обжигает", "купает в лаве", "согревает", "разогревает", "прогревает", "подогревает", "теплит", "раскаляет", "превращает в пепел", "придаёт тепло", "выжигает"]
        rpemodz = ["🦋🌡😵‍💫", "🦋☀️🕺", "🦋🧣💃", "🦋🔥👾", "🦋🌋🙀", "🦋🔥😊", "🦋🔥🔥", "🦋🌞😌", "🦋🔥🎉", "🦋🌡️😎", "🦋🔥😨", "🦋🔥💨", "🦋🔥🧡", "🦋🔥💀"]
    else:
