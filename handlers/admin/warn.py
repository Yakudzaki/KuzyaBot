...
async def warn_handler(message: types.Message):
    ...
    if int(res_w[2]) == int(1):
        ...
        try:
            ...
            await message.answer(f"⚠️ | Пользователь <a href='tg://user?id={res[0]}'>{html.escape(res[2])}</a> получил второе предупреждение, в результате чего был замучен на 24 часа!")
        except:
